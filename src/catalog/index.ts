...
export class CatalogAPI {
	...
	private parseKohaMetadata(kohaData: unknown): CatalogItemMetadata {
		...
		// Extract notes from source.note (can be single object or array)
		let notes: Array<{ type: string; text: string; seq: number }> = [];
		if (data?.source?.note) {
			const noteData = Array.isArray(data.source.note)
				? data.source.note
				: [data.source.note];
	 ...
	 * If no known pattern matches, the raw religion string is returned as-is.
	 */
	private extractParishAndReligion(creators: string[]): {
		...
		for (const creator of creators) {
			...
			if (churchMatch && churchMatch.parish) {
				// churchMatch.religion is already normalize()'d — if unknown,
				// normalize() returns the raw string, so we always get something.
	 ...
	 * ```
	 */
	async getRelatedPlaces(placeId: string): Promise<RelatedPlacesResponse> {
		try {
			...
			// Parse response
			const relatedPlaces: RelatedPlace[] = (response.relatedPlaces || [])
				.map((place) => ({
					placeId: place.placeId || "",
					name: place.name || "",
					fullName: place.fullName,
					relationship: this.parseRelationshipType(
						place.relationship
					),
					note: place.note,
			...
			console.warn(
				`Failed to fetch related places for ${placeId}:`,
				error
			);
			return {
				placeId,
	...
	private parseRelationshipType(
		...
		if (lower.includes("subordinate") || lower.includes("child"))
			return "subordinate";
		if (lower.includes("nearby") || lower.includes("neighbor"))
			return "nearby";
